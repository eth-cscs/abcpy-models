...
class FFSimulator1(ProbabilisticModel, Continuous):
    ...
    def _check_output(self, values):
        if not isinstance(values[0], np.ndarray):
            raise ValueError('Output of Floorflied model is always nparray.')
        return True

    def get_output_dimension(self):
        return 1

    def forward_simulate(self, input_values, k, rng=np.random.RandomState(0)):
        ...
        vector_of_k_samples = self.floorfield(kS, kD, kW, decay, diffusion, k, rng)
        # Format the output to obey API
        result = [np.array([x]).reshape(-1,) for x in vector_of_k_samples]
        return result


    def floorfield(self, kS, kD, kW, decay, diffusion, simRep, rng):
        ...
        if self.entranceType==1:
            floorMap = np.array(list(csv.reader(open("Lcorner.csv"), delimiter=","))).astype("float")
        ...
        if self.entranceType==2:
            floorMap = np.array(list(csv.reader(open("Lcorner_long.csv"), delimiter=","))).astype("float")
            (iSize,jSize) = floorMap.shape
        ...
        for repi in range(simRep):
            ...
            if self.entranceType==2:
                while (numPed < self.totalPed):
                    m, n = rng.randint(startArea[0][0],startArea[0][1]), rng.randint(startArea[1][0],startArea[1][1])
                    if pedPos[m,n]==0:
                        pedPos[m][n] = 1
                        numPed = numPed + 1
                        pedList.append([m,n])
            ...
            while ((numPed < self.totalPed) or (len(pedList) > 0)) and (simTime-inTime < maxTime):
                ...
                if self.entranceType==1:
                    if numPed < self.totalPed:
                        ...
                        while (numPed < self.vFree * corrWidth * self.meshSize * (self.totalPed / self.startSurface) * (simTime - inTime)):
                            m, n = rng.randint(entranceLocation[0], entranceLocation[1]), jSize - 1
                            if pedPos[m][n] == 0:
                                pedPos[m][n] = 1
                                numPed = numPed + 1
                                pedList.append([m, n])
                        ...
                        if floorMap[i][j] != 0:
                            ...
                            if dynamicFF[i][j] > 0:
                                for n in range(0,localDynamicFF):
                                    if rng.uniform(0,1) < decay:
                                        dynamicFF[i][j] = dynamicFF[i][j] - 1
                            # Diffusion
                            if dynamicFF[i][j] > 0 and rng.uniform(0,1) < diffusion:
                                dynamicFF[i][j] = dynamicFF[i][j] - 1
                                index = rng.randint(0,1)
                                if index==0:
                                    dI, dJ = int(np.sign(rng.uniform(-1,1))), 0
                                elif index==1:
                                    dI, dJ = 0, int(np.sign(rng.uniform(-1,1)))
                ...
                for pedID in range(len(pedList)):
                    ...
                    localMap = [None]*3
                    localMap[0] = self._compProb(kS, kD, kW, staticFF[I,J], staticFF[I-1:I+2,J-1:J+2], dynamicFF[I-1:I+2,J-1:J+2], wallFF[I-1:I+2,J-1:J+2])
                    ...
                    if (dI!=0 or dJ!=0) and I-dI>=0 and I-dI<iSize and J-dJ>=0 and J-dJ<jSize:
                        localValue = self._compProb(kS, kD, kW, staticFF[I,J], staticFF[I-dI,J-dJ], dynamicFF[I-dI,J-dJ]-1, wallFF[I-dI,J-dJ])
                    ...
                    if self.motionType==1:
                        ...
                        if totalValue>0:
                            ...
                            else:                       m, n = maxIndex[rng.randint(0,len(maxIndex)-1)]
                ...
                for i in range(iSize):
                    for j in range(jSize-2):
                        ...
                        if nReserved>1:
                            ...
                            chosenIndex, remPed = rng.randint(0,nReserved-1), []
            ...
            repResult += np.array(repSimulationHeatmap).reshape(-1,).tolist()
            repResult += np.array(repSimulationTime).tolist()
            for indadd in range(len(repSimulationPos)):
                repResult += np.array(repSimulationPos[indadd]).reshape(-1,).tolist()
            result.append(copy.deepcopy(repResult))
        return result

    def _compProb(self, kS, kD, kW, sFFcenter, sFF, dFF, wFF):

        def compProb(kS, kD, kW, sFFcenter, sFF, dFF, wFF):
        ...
        result = compProb(kS, kD, kW, sFFcenter, sFF, dFF, wFF)
        return result
...
class FFSimulator2(ProbabilisticModel, Continuous):
    ...
    def __init__(self, parameters, name='FloorField'):
        ...
        if len(parameters) != 7:
            raise RuntimeError(
                'Input list must be of length 7, containing [kS, kD, kW, decay, diffusion, liBufferRatio, liBufferAngle].')
    ...
    def _check_input(self, input_values):
        ...
        if not isinstance(liBufferAngle,
                          (float, np.float64, np.float32, np.float16)) or liBufferAngle < 45 or liBufferAngle > 90:
    ...
    def forward_simulate(self, input_values, k, rng=np.random.RandomState()):
        ...
        # Do the actual forward simulation
        vector_of_k_samples = self.floorfield(kS, kD, kW, decay, diffusion, liBufferRatio, liBufferAngle, k, rng)
        # Format the output to obey API
        result = [np.array([x]) for x in vector_of_k_samples]
        return result

    def floorfield(self, kS, kD, kW, decay, diffusion, liBufferRatio, liBufferAngle, simRep, rng=np.random.RandomState()):
        # Generate n_simulated data
        result = []
        ...
        for repi in range(simRep):
            ...
            if self.entranceType == 2:
                while (numPed < self.totalPed):
                    m, n = rng.randint(startArea[0][0], startArea[0][1]), rng.randint(startArea[1][0],
                                                                                        startArea[1][1])
                    if pedPos[m, n] == 0:
                        pedPos[m][n] = 1
                        numPed = numPed + 1
            ...
            while ((numPed < self.totalPed) or (len(pedList) > 0)) and (simTime - inTime < maxTime):
                ...
                if self.entranceType == 1:
                    if numPed < self.totalPed:
                        warningCounter = 0
                        while (numPed < self.vFree * corrWidth * self.meshSize * (self.totalPed / self.startSurface) * (
                                simTime - inTime)):
                            m, n = rng.randint(entranceLocation[0], entranceLocation[1]), jSize - 1
                            if pedPos[m][n] == 0:
                                pedPos[m][n] = 1
                ...
                for pedID in range(len(pedList)):
                    ...
                    I, dI, J, dJ = pedList[pedID][0], pedDir[pedID][0], pedList[pedID][1], pedDir[pedID][1]
                    localMap = [None] * 3
                    localMap[0] = self._compProb(kS, kD, kW, staticFF[I, J], staticFF[I - 1:I + 2, J - 1:J + 2],
                                           dynamicFF[I - 1:I + 2, J - 1:J + 2], wallFF[I - 1:I + 2, J - 1:J + 2])
                    localMap[1], localMap[2] = pedPos[I - 1:I + 2, J - 1:J + 2], floorMap[I - 1:I + 2, J - 1:J + 2]
                    if (dI != 0 or dJ != 0) and I - dI >= 0 and I - dI < iSize and J - dJ >= 0 and J - dJ < jSize:
                        localValue = self._compProb(kS, kD, kW, staticFF[I, J], staticFF[I - dI, J - dJ], dynamicFF[I - dI, J - dJ] - 1,
                                              wallFF[I - dI, J - dJ])
                        localMap[0][1 - dI][1 - dJ] = localValue.item(0)
                    for n in range(len(localMap)):
                        if J == jSize - 1:
                    ...
                    if self.motionType == 1:
                        ...
                        if totalValue > 0:
                            ...
                            if len(maxIndex) == 1:
                                ...
                                m, n = maxIndex[rng.randint(0, len(maxIndex) - 1)]
                        else:
                            m, n = 1, 1  # special case where transition probability is 0 everywhere
                    elif self.motionType == 2:
                        # Determine next cell where do move stochastically based on previous results
                        p0, p1, p2, p3, p4 = localMap[0][1][1], localMap[0][1][0], localMap[0][1][2], localMap[0][0][1], \
                                             localMap[0][2][1]
                        if totalValue > 0:
                            randValue = rng.uniform(0, 1)
                            if randValue >= (p0 / totalValue) and randValue < ((p0 + p1) / totalValue):
                                ...
                                m, n = 1, 2
                            elif randValue >= ((p0 + p1 + p2) / totalValue) and randValue < (
                                    (p0 + p1 + p2 + p3) / totalValue):
                                m, n = 0, 1
                            elif randValue >= ((p0 + p1 + p2 + p3) / totalValue) and randValue < (
                                    (p0 + p1 + p2 + p3 + p4) / totalValue):
                                m, n = 2, 1
                            else:
                                m, n = 1, 1
                        else:
                ...
                if sum(sum(pedPos[0:iSize - 3, 0:iSize - 3])) > 0 and simTime <= maxTime:
                    ...
                    repSimulationPos.append(copy.deepcopy(pedPos[0:iSize - 3, 0:iSize - 3]))
                    repSimulationTime.append(copy.deepcopy(simTime - tZero))
                    repSimulationHeatmap = repSimulationHeatmap + pedMoved[0:iSize - 3, 0:iSize - 3] * (
                                self.meshSize / self.vFree)
...
class FFSimulator3(ProbabilisticModel, Continuous):
    ...
    def __init__(self, parameters, name='FloorField'):
        ...
        if len(parameters) != 9:
            raise RuntimeError(
                'Input list must be of length 9, containing [kS, kD, kW, decay, diffusion, diasA, diasB, diasP1, diasP2].')
    ...
    def forward_simulate(self, input_values, k, rng=np.random.RandomState()):
        ...
        # Do the actual forward simulation
        vector_of_k_samples = self.floorfield(kS, kD, kW, decay, diffusion, diasA, diasB, diasP1, diasP2, k, rng)
        # Format the output to obey API
        result = [np.array([x]) for x in vector_of_k_samples]
        return result

    def floorfield(self, kS, kD, kW, decay, diffusion, diasA, diasB, diasP1, diasP2, simRep, rng=np.random.RandomState()):
        # Generate n_simulated data
        result = []
        ...
        iTurning, jTurning = iCenter + math.floor(diasB / self.meshSize) - 1, jCenter + math.floor(
            diasA / self.meshSize) - 1
        ...
        for i in range(0, iTurning + 1):
            for j in range(0, jTurning + 1):
                if floorMap[i, j] == 1:
                    ...
                    if iDist == 0:
                        alphaMatrix[i, j] = math.degrees(alpha(math.pi / 2))
                    else:
                        alphaMatrix[i, j] = math.degrees(alpha(math.atan(jDist / iDist)))
        ...
        for repi in range(simRep):
            ...
            while ((numPed < self.totalPed) or (len(pedList) > 0)) and (simTime - inTime < maxTime):
                ...
                if self.entranceType == 1:
                    if numPed < self.totalPed:
                        warningCounter = 0
                        while (numPed < self.vFree * corrWidth * self.meshSize * (
                                self.totalPed / self.startSurface) * (simTime - inTime)):
                            m, n = rng.randint(entranceLocation[0], entranceLocation[1]), jSize - 1
                            if pedPos[m][n] == 0:
                                pedPos[m][n] = 1
                                numPed = numPed + 1
                ...
                for pedID in range(len(pedList)):
                    ...
                    localMap[1], localMap[2] = pedPos[I - 1:I + 2, J - 1:J + 2], floorMap[I - 1:I + 2, J - 1:J + 2]
                    if (dI != 0 or dJ != 0) and I - dI >= 0 and I - dI < iSize and J - dJ >= 0 and J - dJ < jSize:
                        localValue = self._compProb(kS, kD, kW, staticFF[I, J], staticFF[I - dI, J - dJ],
                                              dynamicFF[I - dI, J - dJ] - 1, wallFF[I - dI, J - dJ])
                        localMap[0][1 - dI][1 - dJ] = localValue.item(0)
                    for n in range(len(localMap)):
                        if J == jSize - 1:
                            localMap[n] = np.c_[localMap[n], np.zeros(3)]
                    ...
                    localMap[0][1 - dI][1 - dJ] = 0  # remove back stepping
                    if self.allowStop == 0:        localMap[0][1][
                        1] = 0  # no stopping allowed (depending on options)
                    for i in range(localMap[0].shape[0]):
                        for j in range(localMap[0].shape[1]):
                            if not (i == 1 and j == 1) and localMap[1][i][j] == 1:     localMap[0][i][
                                j] = 0  # exclude occupied positions
                    ...
                    if self.motionType == 1:
                        ...
                        # Determine next cell where do move stochastically based on previous results
                        p0, p1, p2, p3, p4 = localMap[0][1][1], localMap[0][1][0], localMap[0][1][2], \
                                             localMap[0][0][1], localMap[0][2][1]
                        if totalValue > 0:
                            randValue = rng.uniform(0, 1)
                            if randValue >= (p0 / totalValue) and randValue < ((p0 + p1) / totalValue):
                                m, n = 1, 0
                            elif randValue >= ((p0 + p1) / totalValue) and randValue < (
                                    (p0 + p1 + p2) / totalValue):
                                m, n = 1, 2
                            elif randValue >= ((p0 + p1 + p2) / totalValue) and randValue < (
                                    (p0 + p1 + p2 + p3) / totalValue):
