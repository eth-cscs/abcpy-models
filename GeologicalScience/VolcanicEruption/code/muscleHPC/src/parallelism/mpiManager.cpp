...
MpiManager::~MpiManager() {
    if (responsibleForMpiMachine) {
        //if(this->isMainProcessor()) std::cout<<"**** MPI_FINALIZE() called ****"<<std::endl;
        MPI_Finalize();
        ok = false;
        responsibleForMpiMachine = false;
    }else{
        if(globalCommunicator!=MPI_COMM_WORLD && (globalCommunicator!= MPI_COMM_NULL)){
           /* MPI_Errhandler_set(this->globalCommunicator, MPI_ERRORS_RETURN);
            MPI_Comm_free(&this->globalCommunicator);
            MPI_Errhandler_set(this->globalCommunicator, MPI_ERRORS_ARE_FATAL);*/
        }
    }
}

void MpiManager::init(int *argc, char ***argv, MPI_Comm globalCommunicator_, int Mpicolor, bool verbous) {
    if (verbous) {
        std::cerr << "Constructing an MPI thread" << std::endl;
    }
    this->MpiColor=Mpicolor;
    int isMpiAlreadyInitialised;
    MPI_Initialized( &isMpiAlreadyInitialised );
    int ok1=0;
    if (! isMpiAlreadyInitialised){
        ok1 = MPI_Init(argc, argv);
...
void MpiManager::init(MPI_Comm globalCommunicator_, int Mpicolor) {
    globalCommunicator = globalCommunicator_;
    this->MpiColor=Mpicolor;
...
void MpiManager::initOnlyCommunicator(MPI_Comm globalCommunicator_) {
    globalCommunicator = globalCommunicator_;
    ok = true;
}
...
int MpiManager::getMpiColor() const {
    return this->MpiColor;
}

void MpiManager::setMpiColor(int mpiColor){
        this->MpiColor=mpiColor;
}
...
template <>
void MpiManager::receive<char>(char *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<int>(int *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<bool>(bool *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<long long>(long long *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<unsigned long long>(unsigned long long *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<long>(long *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<float>(float *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<double>(double *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<long double>(long double *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
#ifdef PLB_USE_GCC
template <>
void MpiManager::receive<__float128 >(__float128 *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<Complex<float> >(Complex<float> *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<Complex<double> >(Complex<double> *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::receive<Complex<long double> >(Complex<long double> *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
#ifdef PLB_USE_GCC
template <>
void MpiManager::receive<Complex<__float128> >(Complex<__float128> *buf, int count, int source, MPI_Status & status, int tag)
{
    if (!ok) return;
...
template <>
void MpiManager::scatterV<long>(long *sendBuf, int* sendCounts, int* displs,
...
template <>
void MpiManager::scatterV<long double>(long double *sendBuf, int* sendCounts, int* displs,
...
template <>
void MpiManager::scatterV<Complex<float> >(Complex<float> *sendBuf, int* sendCounts, int* displs,
...
template <>
void MpiManager::scatterV<Complex<double> >(Complex<double> *sendBuf, int* sendCounts, int* displs,
...
template <>
void MpiManager::scatterV<Complex<long double> >(Complex<long double> *sendBuf, int* sendCounts, int* displs,
...
template <typename T>
void MpiManager::gather(T* sendBuf, int sendCount, T* recvBuf, int recvCounts, int root){
    this->gather_impl<T>(sendBuf, sendCount,  recvBuf, recvCounts,  root);
}*/

//
template <>
void MpiManager::gather<char>(char* sendBuf, int sendCount,
                                    char* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_CHAR,
                static_cast<void*>(recvBuf), recvCounts, MPI_CHAR,
...
void MpiManager::gather<int>(int* sendBuf, int sendCount,
                                    int* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_INT,
                static_cast<void*>(recvBuf), recvCounts, MPI_INT,
                root, getGlobalCommunicator());

}

/*template <>
void MpiManager::gather<bool>(bool *sendBuf, int sendCount, bool* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount*sizeof(bool), MPI_CHAR,
                static_cast<void*>(recvBuf), recvCounts*sizeof(bool), MPI_CHAR,
                root, getGlobalCommunicator());

}*/

template <>
void MpiManager::gather<long>(long* sendBuf, int sendCount,
                                    long* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_LONG,
                static_cast<void*>(recvBuf), recvCounts, MPI_LONG,
                root, getGlobalCommunicator());

}

template <>
void MpiManager::gather<float>(float* sendBuf, int sendCount,
                                    float* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_FLOAT,
                static_cast<void*>(recvBuf), recvCounts, MPI_FLOAT,
                root, getGlobalCommunicator());

}
template <>
void MpiManager::gather<double>(double* sendBuf, int sendCount,
                                    double* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_DOUBLE,
                static_cast<void*>(recvBuf), recvCounts, MPI_DOUBLE,
                root, getGlobalCommunicator());

}

template <>
void MpiManager::gather<long long>(long long* sendBuf, int sendCount,
                                    long long* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_LONG_LONG,
                static_cast<void*>(recvBuf), recvCounts,  MPI_LONG_LONG,
                root, getGlobalCommunicator());

}

template <>
void MpiManager::gather<unsigned long long>(unsigned long long* sendBuf, int sendCount,
                                    unsigned long long* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_UNSIGNED_LONG_LONG,
                static_cast<void*>(recvBuf), recvCounts,  MPI_UNSIGNED_LONG_LONG,
                root, getGlobalCommunicator());

}

template <>
void MpiManager::gather<long double>(long double* sendBuf, int sendCount,
                                    long double* recvBuf, int recvCounts, int root){
    if (!ok) return;
    MPI_Gather(static_cast<void*>(sendBuf), sendCount, MPI_LONG_DOUBLE,
                static_cast<void*>(recvBuf), recvCounts, MPI_LONG_DOUBLE,
                root, getGlobalCommunicator());
...
/*template <typename T>
void MpiManager::gatherV(T* sendBuf, int sendCount, T* recvBuf, int* recvCounts,
                  int* displs, int root){

    this->gatherv_impl<T>(sendBuf,  sendCount,  recvBuf,  recvCounts,
                          displs, root);
}*/
...
/// <<warning>>>: I did not test the bool version of gatherV
/*template <>
                                    ...
                                    int root)
{
    ...
    // Adpt displs and recvCounts from bool to char
    int *recvCounts_tmp= (int*) malloc(gsize * sizeof(int));
    int *displs_tmp= (int *)malloc(gsize * sizeof(int));

    for(size_t i=0; i< gsize; i++){
        recvCounts_tmp[i]=recvCounts[i]*sizeof(bool);
        displs_tmp[i]=displs[i]*sizeof(bool);
    }
    MPI_Gatherv(static_cast<void*>(sendBuf), sendCount*sizeof(bool), MPI_CHAR,
                static_cast<void*>(recvBuf), recvCounts_tmp, displs_tmp, MPI_CHAR,
                root, getGlobalCommunicator());
...
template <>
void MpiManager::gatherV<long>(long* sendBuf, int sendCount,
...
template <>
void MpiManager::gatherV<long long>(long long* sendBuf, int sendCount,
                                           long long* recvBuf, int* recvCounts, int* displs,
                                           int root)
{
    if (!ok) return;
    MPI_Gatherv(static_cast<void*>(sendBuf), sendCount, MPI_LONG_LONG,
                static_cast<void*>(recvBuf), recvCounts, displs, MPI_LONG_LONG,
                root, getGlobalCommunicator());
}

template <>
void MpiManager::gatherV<unsigned long long>(unsigned long long* sendBuf, int sendCount,
                                           unsigned long long* recvBuf, int* recvCounts, int* displs,
                                           int root)
{
    if (!ok) return;
    MPI_Gatherv(static_cast<void*>(sendBuf), sendCount, MPI_UNSIGNED_LONG_LONG,
                static_cast<void*>(recvBuf), recvCounts, displs, MPI_UNSIGNED_LONG_LONG,
                root, getGlobalCommunicator());
}


template <>
void MpiManager::gatherV<long double>(long double* sendBuf, int sendCount,
...
#ifdef PLB_USE_GCC
template <>
void MpiManager::gatherV<__float128 >(__float128 * sendBuf, int sendCount,
...
template <>
void MpiManager::gatherV<Complex<float> >(Complex<float> * sendBuf, int sendCount,
...
template <>
void MpiManager::gatherV<Complex<double> >(Complex<double> * sendBuf, int sendCount,
...
template <>
void MpiManager::gatherV<Complex<long double> >(Complex<long double> * sendBuf, int sendCount,
...
#ifdef PLB_USE_GCC
template <>
void MpiManager::gatherV<Complex<__float128> >(Complex<__float128> * sendBuf, int sendCount,
...
template <>
void MpiManager::reduceVect<unsigned long long>(std::vector<unsigned long long>& sendVal,
                                       std::vector<unsigned long long>& recvVal, MPI_Op op, int root)
{
    if (!ok) return;
    if (sendVal.empty()) return;
    void* recvPtr;
    ...
    MPI_Reduce(static_cast<void*>(&(sendVal[0])), recvPtr,
               sendVal.size(), MPI_UNSIGNED_LONG_LONG, op, root, getGlobalCommunicator());
}
