...
int main( int argc, char **argv ) {
  ...
  const double DH = 0.5;
	...
	const piaf::Double3 speed = { 1.0, 1.0, 1.0 };
	const double simTime = 10.0;

	//std::cout << "ur : " << sqrt( ( 4.0 * D ) / dt ) << std::endl;

	std::shared_ptr< piaf::DirectionalDiffusion > diffusion(new piaf::DirectionalDiffusion( DV, DH ) );
  ...
  /*domain.insertParticles( { 10.0, 0.0, 0.0 }, 10, 0 );
  domain.insertParticles( { 0.0, 20.0, 0.0 }, 20, 0 );
  domain.insertParticles( { 0.0, 0.0, 30.0 }, 30, 0 );*/
  ...
  //std::cout << domain.getNParticleAt( { 60, 50, 50 } ) << std::endl;

	piaf::FileManager fm;
	fm.write( "test-3d-directional.h5", &domain );
