...
class Diffusion: public SpeedFunctor{

public:

	Diffusion( double d ): SpeedFunctor("piaf_diffusion"), d_ ( d )
	{
...
inline void Diffusion::setD( double d ){
	d_ = d;
}

inline Double3 Diffusion::operator()(Double3 pos, double t, double dt, std::shared_ptr<GenericParticleFamily> family){
	//Double3 speed;

	// 2D
	//double ur = sqrt( ( 4.0 * d_ ) / dt );
	// 3D
	double ur = sqrt( ( 6.0 * d_ ) / dt );
