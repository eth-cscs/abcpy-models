...
globals [ empirical_data_m empirical_data_c food_per_patch reference_T Boltz T Arrhenius adult_size juvenile_size cocoon_size ]

patches-own [ current_food ]

turtles-own 
[ mass                              ; individual mass (g)
  BMR                               ; energy cost of maintenance (kJ)
  food_ingested                     ; whether or not energy was ingested
...
to setup-interface
  set initial_number_juveniles 10   ; number of juveniles to start the simulation with
  set initial_number_adults 0       ; number of adults to start the simulation with
  set total_food 40                 ; total amount of food initially available (g)
  set scape_size 0.0144             ; size of the scape (m)
  set temperature 25                ; temperature (C)
end
...
to setup
  ...
  ;; from Reinecke & Viljoen 1990
  set empirical_data_m [ 0.32 0.35 0.36 0.30 0.30 0.27 0.16 0.47 0.44 0.44 0.41 0.32 0.30 0.14 0.05 0.35 0.66 ]
  set empirical_data_c [ 0  8 34 23 10  0  1 30 46 32 12  2  0  0  0  0 24 54 30 ]
...
to update-experiment
  ...
  if ticks = 25 [ ask patches [ set current_food (10 / count patches) ] ]
  if ticks = 35 [ ask patches [ set current_food current_food + (10 / count patches) ] ]
  if ticks = 85 [ ask patches [ set current_food current_food + (50 / count patches) ] ]
  if ticks = 125 [ ask patches [ set current_food current_food - (25 / count patches) ] ]
  if ticks = 165 [ ask patches [ set current_food current_food + (25 / count patches) ] ]
end
...
to-report sum-hatchlings
  ifelse any? turtles with [ breed != cocoons ]
  [ report sum [ hatchlings ] of turtles with [ breed != cocoons ] ]
  [ report 0 ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
true
false
"" "if ticks = 25 or ticks > 25 and remainder ticks 10 = 5 and ticks <= 175\n[ set-current-plot-pen \"model  \"\n  plot mean-mass\n  set-current-plot-pen \"data  \"\n  plot item ((ticks - 25) / 10) empirical_data_m ]"
PENS
...
true
false
"" "if ticks = 25 or ticks > 25 and remainder ticks 10 = 5 and ticks <= 205\n[ set-current-plot-pen \"model  \"\n  plot sum-hatchlings\n  set-current-plot-pen \"data  \"\n  plot item ((ticks - 25) / 10) empirical_data_c ]"
PENS
"data  " 1.0 0 -16777216 true "" ""
"model  " 1.0 0 -7500403 true "" ""
...
The full version of the earthworm IBM it runs was originally described here:
...
Reinecke, A.J. & Viljoen, S.A. "The influence of feeding patterns on growth and reproduction of thevermicomposting earthworm Eisenia fetida (Oligochaeta)" (1990)
