...
#Identity statistcs is used for distances which is computed on the whole dataset
...
fake_output_data_1 = pollak.forward_simulate([0.5, 3, 3, 8, 12.0, 2.1],10)
# fake_output_data_2 = pollak.forward_simulate([0.5, 3, 3.1, 8, 12.0, 2.1],100)
#
# print(distance_calculator.distance(fake_output_data_1, fake_output_data_2))

## SABC ##
sampler_sabc =  SABC([pollak], [distance_calculator], backend, kernel)
steps, epsilon, n_samples, n_samples_per_param, beta, delta, v, ar_cutoff, resample, n_update, full_output =2, 10.0, 10, 10, 2, 0.2, 0.3, 0.1, None, None, 1
# #adaptcov = 1
print('SABC Inferring')
journal_sabc = sampler_sabc.sample( [fake_output_data_1], steps, epsilon, n_samples, n_samples_per_param, beta, delta, v, ar_cutoff, resample, n_update, full_output)
print('SABC done')
