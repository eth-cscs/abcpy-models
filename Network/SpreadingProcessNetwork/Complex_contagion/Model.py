...
class Diffusion_model_complex_contagion(Model):
    ...
    def simulate(self, n_simulate):
        ...
        for k in range(n_simulate):
            ...
            if 0 in self.time_observed:                
                diffusionstate_array_tmp.append([present_infected_nodes,copy.deepcopy(exposed_nodes),copy.deepcopy(count_of_exposure)])
                    ...
                    if (chosen_node_for_exposure in infected_nodes) == 0:
                        ...
                        if self.rng.binomial(1,self.theta) == 1:
                            if (chosen_node_for_exposure in exposed_nodes):
                                ...
                                [len(set(infected_nodes).intersection(self.network.neighbors(chosen_node_for_exposure)))-1] += 1
                            else:
                                exposed_nodes.append(chosen_node_for_exposure)
                                count_of_exposure.append(list(np.zeros(shape=(len(self.network.neighbors(chosen_node_for_exposure)),))))
                                count_of_exposure[exposed_nodes.index(chosen_node_for_exposure)]\
                                [len(set(infected_nodes).intersection(self.network.neighbors(chosen_node_for_exposure)))-1] += 1 
