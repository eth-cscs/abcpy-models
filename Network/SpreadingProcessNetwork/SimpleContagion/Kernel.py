import numpy as np
from abcpy.distributions import Distribution, multivariate_normal
...
class DiffusionKernel(Distribution):
    ...
    def __init__(self, network, mean=0, var=1, seed_node=1, seed=None):
        """
    ...
    def sample(self, k):
        ...
        samples_1 = self.rng.normal(self.mean, self.var, k)
        ...
        return np.transpose(np.reshape(np.append(samples_1, samples_2), (2, k)))

    def pdf(self, x):
        if x[1] in self.network.neighbors(self.seed_node):
            return norm(self.mean, self.var).pdf(x[0]) * (1 / len(self.network.neighbors(self.seed_node)))
        else:
            return 0
